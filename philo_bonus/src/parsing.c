...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/08/07 16:14:07 by sgadinga          #+#    #+#             */
/*   Updated: 2025/08/22 14:49:49 by sgadinga         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <philo_bonus.h>

t_arg_desc	define_argument(const char *name, t_arg_type type, void *target,
		int max_value)
{
	return ((t_arg_desc){name, type, target, max_value});
}

t_arg_desc	*define_default_arguments(t_table *table)
{
	static t_arg_desc	args[5];

	args[0] = define_argument("n_philo", TYPE_UINT, &table->n_philo, 200);
	args[1] = define_argument("time_to_die", TYPE_TIME, &table->time_to_die_ms,
			0);
	...
	args[4] = define_argument("max_meals", TYPE_UINT, &table->max_meals, 0);
	return (args);
}
	...
	while (i < argc)
	{
		...
		if (args[i - 1].max_value && value > args[i - 1].max_value)
			arg_error(args[i - 1].name, i, "exceeds maximum allowed value",
				&ret);
		if (args[i - 1].type == TYPE_TIME && value < 60)
			arg_error(args[i - 1].name, i, "must be >= 60ms", &ret);
