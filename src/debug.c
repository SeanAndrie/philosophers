...
/*   Updated: 2025/07/29 19:27:14 by sgadinga         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	log_status(t_philo *philo, char *action, char *ansi_color)
{
	...
	mutex_gate(&philo->table->mutexes.log_lock, LOCK, "log");
	if (ansi_color)
		printf("%s%ld %u %s\033[0m\n", ansi_color, time, philo->id, action);
	else
		printf("%ld %u %s\n", time, philo->id, action);
	mutex_gate(&philo->table->mutexes.log_lock, UNLOCK, "log");
}
